...
//----------------------
//jjeong
/* input tid should be child of current running thread
 * return child thread of current running thread
 * and it should be correspond with input tid
*/
struct thread* get_child(tid_t tid){
	...
	struct thread *ct;
	 for (e = list_begin (&pt->children); e != list_end (&pt->children);
           e = list_next (e))
        {
        	ct = list_entry (e, struct thread, child_elem);
        
		if(ct != NULL && ct->tid == tid)// if this child exit yet
			break;
		else
			ct = NULL ;
        }
...
/*
 * runnig thread willd be blocked 
 * until child thread corresponding
...
void children_remove(struct thread *thread){
	...
	for(struct list_elem *e=list_begin(&thread->children); e != list_end(&thread->children);e=list_next(e) ){
		ct = list_entry (e, struct thread, child_elem);
...
//jjeong
  ...
  if(strcmp(name,"main")){
	  //jjeong 
	  /* store parent thread.. 
	   * this cureent runnig thread 
	   * would be parent thread of generated thread
